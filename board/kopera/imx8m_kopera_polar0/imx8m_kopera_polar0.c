...
#define BACK_KEY		IMX_GPIO_NR(3, 22)
...
static iomux_v3_cfg_t const back_pads[] = {
	IMX8MQ_PAD_SAI5_RXD1__GPIO3_IO22 | MUX_PAD_CTRL(BACK_PAD_CTRL),
};

int board_gpio_init(void)
{
	/* We are only interested in the '+' key i.e. volume up key.
	   We use it as a signal to check for firmware on USB.
	*/
	...
	/* This is here for future use (if any) */
...
{
	env_set("board_name", "IMX8M-KOPERA-POLAR0");
	env_set("board_rev", "polar0");
	...
	if (gpio_get_value(VOLUME_UP_KEY) == 1){
		printf("Update key was pressed during boot...\nEnabling factory update from USB disk\n");
		if (env_set("script", "boot.scr") != 0){
			printf("ERROR : Failed to enable factory update from USB disk !\n");
		}
	}
	else {
		if (env_get("script")){
			if (env_set("script", NULL) !=0){
				printf("ERROR : Failed to disable factory update from USB disk !\n");
			}
		}
	}
